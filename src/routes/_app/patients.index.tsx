  ...
  return (
    <>
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex items-end justify-between flex-wrap gap-4">
        <div>
          <div className="font-mono text-xs text-primary uppercase tracking-widest">Module · Patients</div>
        ...
        <div className="mt-12 text-center text-muted-foreground">
          {patients.length === 0 ? (
            <>
              ...
              <Link to="/patients/new" className="text-primary hover:underline">Créez le premier dossier</Link>.
            </>
          ) : (
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((p, i) => (
            <motion.div
              ...
              whileHover={{ y: -4 }}
              onClick={() => navigate({ to: "/patients/$patientId", params: { patientId: String(p.id) } })}
              className="glass rounded-2xl p-5 cursor-pointer relative overflow-hidden"
            >
              <div className="absolute top-0 right-0 h-20 w-20 rounded-full blur-2xl opacity-20" style={{ background: "var(--primary)" }} />
              <div className="relative flex items-start gap-4">
                <div className="h-12 w-12 rounded-xl flex items-center justify-center font-display font-bold flex-shrink-0 bg-primary/15 text-primary">
                  {(p.prenom[0] + p.nom[0]).toUpperCase()}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="font-display font-semibold truncate">{p.prenom} {p.nom}</div>
                  <div className="text-xs text-muted-foreground font-mono mt-0.5">{p.numero_dossier}</div>
