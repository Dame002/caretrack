...
function NewPatient() {
  ...
  const [form, setForm] = useState({
    ...
    telephone: "", contact_urgence: "", antecedents: "",
  });
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setBusy(true);
    try {
      const res = await api.post<{ patient: { id: number; numero_dossier: string } }>("/patients", {
        nom: form.nom,
        prenom: form.prenom,
      ...
      navigate({ to: "/patients/$patientId", params: { patientId: String(res.patient.id) } });
    } catch (err: any) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="max-w-2xl">
      ...
      <div className="font-mono text-xs text-primary uppercase tracking-widest">UC-01 · Enregistrement</div>
      <h1 className="mt-2 font-display text-4xl font-bold">Nouveau patient</h1>
      <p className="mt-1 text-sm text-muted-foreground">Un passage est créé automatiquement à l'enregistrement.</p>
      ...
      <form onSubmit={submit} className="mt-8 glass rounded-2xl p-6 space-y-4">
        ...
        <Input label="Date de naissance" type="date" required value={form.date_naissance} onChange={(v) => set("date_naissance", v)} />
        <Input label="Téléphone" value={form.telephone} onChange={(v) => set("telephone", v)} placeholder="+221 ..." />
        <Input label="Contact d'urgence" value={form.contact_urgence} onChange={(v) => set("contact_urgence", v)} placeholder="Nom + téléphone" />
        <div>
          <label className="text-xs uppercase tracking-wider text-muted-foreground">Antécédents médicaux</label>
          <textarea
